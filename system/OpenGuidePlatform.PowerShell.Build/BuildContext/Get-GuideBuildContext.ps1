function Get-GuideBuildContext {
    ...
    if($EventName -eq 'pull_request'){
        ...
        return [pscustomobject]@{Ring='Canary';Target='canary';DeploymentEnvironment="canary-$PullRequestNumber"}
    }
    if($EventName -eq 'merge_group'){
        return [pscustomobject]@{Ring='Canary';Target='canary';DeploymentEnvironment='canary-merge-group'}
    }
    switch -CaseSensitive ($PreReleaseLabel){
        ...
        default { [pscustomobject]@{Ring='Canary';Target='canary';DeploymentEnvironment='canary'} }
    }
}