...
Node.js 20 or newer and npm are required when declarations exist. The same root build operation restores the lockfile-verified Playwright package and Chromium to `.processing/browser-tools/` locally and in CI. Deep Windows checkouts use a short temporary cache keyed by workspace and lockfile to keep Chromium executable paths within Windows process limits. The chosen path and browser stdout/stderr are retained in the validation logs. The first run needs download access; later runs reuse the cache. Linux requires Chromium system libraries; use a supported runner image. No tools or browser settings are installed globally.
