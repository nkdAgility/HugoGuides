...
- [x] Complete coordinated release metadata and adoption updates for the native Hugo dependency, component versions, schemas, toolchain requirements, workflow identity and generated skills/instruction files. Managed machine enforcement remains E06. Immutable provenance may record source commits; action references must use version tags with no unnecessary version restriction.
...
- [ ] Reconcile existing consumer-owned files explicitly during adoption; do not overwrite them or claim a clean fixture installation proves migration of existing sites. **E09–E11 adoption gate:** retained here for traceability, outside E07 fresh-fixture acceptance.
...
### E07 end-to-end installed-consumer acceptance

The actual package from `053ff5b9e2b509fc97689d0109b39b00b2586c06` and matching canonical Go pseudo-version installed successfully in a fresh disposable consumer fixture. Both installed preview and production builds passed. A cached preview build passed with GitHub release transport blocked and GOPROXY=off, correctly warning that freshness was unavailable. All seven skills were installed, their version-locked Core module loaded, and Get-GuideInventory returned the sample's actual guide/edition states. A same-release update also succeeded. `baselines/2026-09-13-module-identity/installed-consumer.json` records the package manifest and hashes of the logs and passing artifact reports.

Only release download transport was substituted with local immutable files; package content, checksum verification, installer, native Go resolution and installed builds were real. Named release publication remains E08. The earlier failed rehearsal and the Windows browser fix remain recorded; the successful run used a fresh fixture and package, not a patched installed cache. The disposable cleanup branch was removed only after confirming its commit had not changed.

Actual existing-site conflict reconciliation belongs to E09–E11, where each bespoke site gets its own adoption PR. E07 proves refusal/preservation and fresh-fixture installation; it does not silently overwrite or claim to migrate the real sites. This clarifies the existing follow-up entry without removing the adoption requirement.


### E07 closed — 2026-09-13
