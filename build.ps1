...
param(
    ...
    [ValidateSet('auto','local','canary','preview','production')][string]$Target='local',
    [string]$WorkspaceRoot=$PSScriptRoot,
    [string]$PolicyPath,[string]$SourcePath='site',
    [string]$OutputPath,
    [string]$Version,
    [string]$DeliveryContextPath,[int]$PullRequestNumber,[string]$BaseUrl,[string]$ReleaseTag,[string]$DeploymentUrl,[string]$DeploymentEnvironment,
...
if($Product -eq 'GuideSite'){
    ...
    Invoke-GuideSiteBuild -SourcePath $SourcePath -DeliveryContextPath $DeliveryContextPath -PullRequestNumber $PullRequestNumber -Stage $Stage -Target $Target -WorkspaceRoot $WorkspaceRoot -PolicyPath $PolicyPath -OutputPath $OutputPath -Version $Version -BaseUrl $BaseUrl -DeploymentUrl $DeploymentUrl -DeploymentEnvironment $DeploymentEnvironment -Deploy:$Deploy -DeploymentAdapter $DeploymentAdapter
}else{
