...
Describe 'Module freshness evidence' {
    It 'reports a current pin as informational' {
        Mock Invoke-GuideGoModuleQuery -ModuleName OpenGuidePlatform.PowerShell.Build { [pscustomobject]@{Version='v0.8.4'} }
    ...
    It 'warns about a different latest version without requesting an update' {
        Mock Invoke-GuideGoModuleQuery -ModuleName OpenGuidePlatform.PowerShell.Build {
    ...
    It 'does not claim freshness when the registry is unavailable or a replacement is active' {
        Mock Invoke-GuideGoModuleQuery -ModuleName OpenGuidePlatform.PowerShell.Build { throw 'Registry unavailable' }
        (Get-GuideModuleFreshness $TestDrive).Code | Should -Be MODULE_FRESHNESS_UNAVAILABLE
        Mock Invoke-GuideGoModuleQuery -ModuleName OpenGuidePlatform.PowerShell.Build { [pscustomobject]@{Version='v0.8.4';Replace=@{Path='../local'}} }
