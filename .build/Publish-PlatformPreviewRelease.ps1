...
if($LASTEXITCODE -eq 0){
    ...
    & gh release download $tag --repo $env:GITHUB_REPOSITORY --pattern OpenGuidePlatform.zip --pattern release-manifest.json --dir $verify
...
& gh release create $tag "$OutputPath/OpenGuidePlatform.zip" "$OutputPath/release-manifest.json" --repo $env:GITHUB_REPOSITORY --target $commit --prerelease --latest=false --title "OpenGuidePlatform $($manifest.version)" --notes-file "$OutputPath/release-notes.md"
